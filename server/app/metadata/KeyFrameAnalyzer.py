    ...
    async def analyze(self) -> None:
        ...
        try:
            ...
            options = [
                ...
                '-select_streams', 'v:0',
                '-show_packets',
                # MPEG-TS でなければ時間で効率よくシークできるためパケットの位置は表示しない
                # MPEG-TS でなければ time_base も表示
            ...
            for packet in packets:
                ...
                if 'flags' not in packet or 'dts' not in packet or (self.container_format == 'MPEG-TS' and 'pos' not in packet):
                    continue
                # キーフレームのみを抽出。ただし、最後のフレームが非キーフレームの場合、シーク可能性のために追加
                if 'K' in packet['flags'] or packet is packets[-1]:
                    if self.container_format == 'MPEG-TS':
                        ...
                        # offset は添え字と同値。dts は time_base を 90000Hz に変換
                        key_frames.append({
