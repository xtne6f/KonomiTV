...
class TSInfoAnalyzer:
    ...
    def __init__(self, recorded_video: schemas.RecordedVideo, end_ts_offset: int | None = None) -> None:
        ...
        if self.recorded_video.container_format == 'MPEG-TS':
            ...
            self.ts = TSInfoAnalyzer.TransportStreamFileWorkaround(BytesIO(packets))
                ...
                if abs_time == 0xffffffff:
                    ...
                    for i in range(n):
                        ...
                        if code < 0 or code >= len(pids):
                            return False
                        if dict[code] is not None and not callback(time_sec, pids[code], dict[code]):
                            return False
    ...
    # TODO: 物理ファイル以外を受け取れるよう ariblib を変更すべき。このやり方は ariblib の内部実装を仮定しているのでよくない
